...
class RateLimiterMySQL extends RateLimiterStoreAbstract {
  ...
  _getConnection() {
    return new Promise((resolve) => {
      ...
      const int = this._getInternalConnection();
      int.then((conn) => {
        // detect if this is a mariadb connection
        if (conn.info !== undefined && conn.info.serverVersion !== undefined && conn.info.serverVersion.mariaDb === true) {
          try {
            // try to get callback compat library
            // https://github.com/mariadb-corporation/mariadb-connector-nodejs/blob/master/documentation/callback-api.md
            // eslint-disable-next-line global-require,node/no-missing-require,import/no-unresolved,import/extensions
            const mariaCb = require('mariadb/callback');
            // const {_events, _eventsCount, _maxListners, typeCast, ...rest} = conn.opts;
            // const callbackConn = mariaCb.createConnection(rest);
            const callbackConn = mariaCb.createConnection(conn.opts);
            this.conn = callbackConn;
            resolve(callbackConn);
          } catch (e) {
            // it didn't work! maybe they are using a different library? YOLO
            this.conn = conn;
            resolve(conn);
          }
        } else {
